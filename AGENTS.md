...
- Do not add WebUI, album, whole-firmware OTA, Home Assistant, deep sleep, or an SD dependency for applications. The user-requested SD Wi-Fi JSON boot source is supported; it is read-only. Independent per-app A/B updates are supported.
